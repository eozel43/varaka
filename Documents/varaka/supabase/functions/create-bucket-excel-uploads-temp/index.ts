...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.ms-excel", "text/csv"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for excel-uploads" ON storage.objects FOR SELECT USING (bucket_id = 'excel-uploads');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for excel-uploads" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'excel-uploads');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for excel-uploads" ON storage.objects FOR UPDATE USING (bucket_id = 'excel-uploads');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for excel-uploads" ON storage.objects FOR DELETE USING (bucket_id = 'excel-uploads');`
    ];
